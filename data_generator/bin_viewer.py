...
def directly_interpolation():
    p000 = read_bin('../data/blend2/000/sphere_up_output.bin')
    p010 = read_bin('../data/blend2/010/sphere_up_output.bin')
    for i in [0.1 * x for x in range(0, 11, 1)]:
        p = p010 * i + (1 - i) * p000
        p = p * 10
        p = p.astype('uint8')
        img = Image.fromarray(p)
        img.save('../data/blend2/%s.jpg' % i)
